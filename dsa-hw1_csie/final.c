...
long long int myFloor(long long int a,long long int b );//自己寫的floor函數,添加除法功能 
...
long long int myFloor(long long int a,long long int b)
{
	if(a>0&&b>0)return a/b; //c 整數除法本身就是無條件捨去,只是複數才會變 
	if(a<0&&b<0)return (-a)/(-b);
	if(a<0&&b>0)
	{
	 a = -a;//改正
	 if(a%b==0)return (a/b)*(-1);//整除就不用扣一 
	 else return (a/b)*(-1)-1;//先都正整互除->乘上負號->多剪掉一 
	}
	if(a>0&&b<0)
	{
	 b = -b;//改正
